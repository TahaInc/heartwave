...
2. The device turns the "active pulse reading" light on.
3. The device reads the user's heart rate.
4. The device displays the current session metrics (see UC6).
5. The device repeats steps 3 and 4 until stopped by the user, waiting 1 second between each repetition.
6. The user presses the selector again to end the session.
7. The device turns the "active pulse reading" light off.
8. The device saves the current session.
9. The device displays the results of the session (see UC7).
...
* 3a. There is no heart rate data available.
  * 3a1. The session is automatically ended.
  * 3a2. Steps 6 and 7 are executed.
...
3. The device calculates and displays the coherence score on the display and on the coloured lights.
